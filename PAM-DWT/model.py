...
class knowledge_adaptation_convnext(nn.Module):
    def __init__(self):
        ...
        checkpoint = torch.load('./trained_models/convnext_xlarge_22k_1k_384_ema.pth')
...
class dwt_ffc_UNet2(nn.Module):
    def __init__(self, output_nc=3, nf=16):
        ...
        pre_process = [nn.Conv2d(3, 32, 3, padding=1)]
        pre_process_2 = [nn.Conv2d(3, 64, 3, padding=1)]
        self.SFT_scale_conv0 = nn.Conv2d(32, 16, 1)
        self.SFT_scale_conv1 = nn.Conv2d(16, 32, 1)
        self.SFT_shift_conv0 = nn.Conv2d(32, 16, 1)
        self.SFT_shift_conv1 = nn.Conv2d(16, 32, 1)

        self.pre = nn.Sequential(*pre_process)
        self.pre_2 = nn.Sequential(*pre_process_2)
        self.SFT_scale_conv0_1 = nn.Conv2d(3, 3, 1)
        self.SFT_scale_conv1_1 = nn.Conv2d(3, 3, 1)
        self.SFT_shift_conv0_1 = nn.Conv2d(3, 3, 1)
        self.SFT_shift_conv1_1 = nn.Conv2d(3, 3, 1)

        self.SFT_scale_conv0_2 = nn.Conv2d(64, 32, 1)
        self.SFT_scale_conv1_2 = nn.Conv2d(32, 64, 1)
        self.SFT_shift_conv0_2 = nn.Conv2d(64, 32, 1)
        self.SFT_shift_conv1_2 = nn.Conv2d(32, 64, 1)
        ...
        # nn.init.zeros_(self.SFT_shift_conv0_1.weight)
        # nn.init.zeros_(self.SFT_shift_conv1_1.weight)
        # nn.init.zeros_(self.SFT_scale_conv0_1.weight)
        # nn.init.zeros_(self.SFT_scale_conv1_1.weight)
        # nn.init.zeros_(self.SFT_shift_conv0_2.weight)
        # nn.init.zeros_(self.SFT_shift_conv1_2.weight)
        # nn.init.zeros_(self.SFT_scale_conv0_2.weight)
        # nn.init.zeros_(self.SFT_scale_conv1_2.weight)
    ...
    def forward(self, input):
        ...
        conv_start_prior = self.initial_conv(xx)

        conv_start = self.bn1(conv_start)
        conv_start_prior = self.bn1(conv_start_prior)

        conv_out1 = self.layer1(conv_start)
        conv_out1_prior = self.layer1(conv_start_prior)

        dwt_low_0, dwt_high_0 = self.DWT_down_0(x)
        dwt_low_0_p, dwt_high_0_p = self.DWT_down_0(xx)

        out1 = torch.cat([conv_out1, dwt_low_0], 1)
        out1_p = torch.cat([conv_out1_prior, dwt_low_0_p], 1)

        conv_out2 = self.layer2(out1)
        conv_out2_p = self.layer2(out1_p)

        dwt_low_1, dwt_high_1 = self.DWT_down_1(out1)
        dwt_low_1_p, dwt_high_1_p = self.DWT_down_1(out1_p)

        out2 = torch.cat([conv_out2, dwt_low_1], 1)
        out2_p = torch.cat([conv_out2_p, dwt_low_1_p], 1)

        conv_out3 = self.layer3(out2)
        conv_out3_p = self.layer3(out2_p)

        dwt_low_2, dwt_high_2 = self.DWT_down_2(out2)
        dwt_low_2_p, dwt_high_2_p = self.DWT_down_2(out2_p)

        out3 = torch.cat([conv_out3, dwt_low_2], 1)
        out3_p = torch.cat([conv_out3_p, dwt_low_2_p], 1)

        scale_2 = self.SFT_scale_conv1_2(F.leaky_relu(self.SFT_scale_conv0_2(out3_p), 0.1, inplace=True))
        shift_2 = self.SFT_shift_conv1_2(F.leaky_relu(self.SFT_shift_conv0_2(out3_p), 0.1, inplace=True))

        out3 = out3 * (scale_2 + 1) + shift_2  # 1

        out3_ffc = self.FFCResNet(out3)

        out3_ffc = out3_ffc * (scale_2 + 1) + shift_2  # 2
        ...
        tail1 = tail1 * (scale + 1) + shift
        tail2 = self.bn2(tail1)
        scale_1 = self.SFT_scale_conv1_1(F.leaky_relu(self.SFT_scale_conv0_1(input[1]), 0.1, inplace=True))
        shift_1 = self.SFT_shift_conv1_1(F.leaky_relu(self.SFT_shift_conv0_1(input[1]), 0.1, inplace=True))
        dout1 = self.tail_conv2(tail2)
        dout1 = dout1 * (scale_1 + 1) + shift_1
        return dout1
...
class fusion_net(nn.Module):
    def __init__(self):
        ...
        self.SFT_scale_conv0 = nn.Conv2d(3, 3, 1)
        self.SFT_scale_conv1 = nn.Conv2d(3, 3, 1)
        self.SFT_shift_conv0 = nn.Conv2d(3, 3, 1)
        self.SFT_shift_conv1 = nn.Conv2d(3, 3, 1)

    def forward(self, input):
        dwt_branch = self.dwt_branch(input)
        knowledge_adaptation_branch = self.knowledge_adaptation_branch(input[0])
