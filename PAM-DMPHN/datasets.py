...
class NH_HazeDataset(Dataset):
    def __init__(self, hazed_image_files, dehazed_image_files, root_dir, crop=False, crop_size=256, multi_scale=False,
    ...
    def __getitem__(self, idx):
        ...
        name = image_name[2].split('_haze')[0] + '.png'
        name_prior = image_name[2].split('_haze')[0] + '_prior.png'
        hazed_image = Image.open(os.path.join(self.root_dir, image_name[0], image_name[1], image_name[2])).convert(
            'RGB')
        dehazed_image = Image.open(os.path.join(self.root_dir, image_name[0], 'clear', name)).convert(
            'RGB')  # change the filename
        prior_image = Image.open(os.path.join(self.root_dir, image_name[0], 'prior', name_prior, )).convert(
            'RGB')
        ...
        if self.transform:  # None
            ...
            prior_image = self.transform(prior_image)
        ...
        if self.multi_scale:  # False
            ...
            return {'hazed_image_s1': hazed_image_s1, 'hazed_image_s2': hazed_image_s2,
                    'hazed_image_s3': hazed_image_s3, 'dehazed_image_s1': dehazed_image_s1,
                    'dehazed_image_s2': dehazed_image_s2, 'dehazed_image_s3': dehazed_image_s3}
        else:
            return {'hazed_image': hazed_image, 'prior_image': prior_image, 'dehazed_image': dehazed_image,
                    'haze_name': image_name[2], 'clear_name': name, 'prior_name': name_prior}
